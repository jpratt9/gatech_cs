    ...
    def observe(self, observation, gameState):
        ...
        if not noisyDistance == None:
            for loc in self.legalPositions:
                ...
                allPossible[loc] = self.beliefs[loc] * emissionModel[dist]
        else:
            allPossible = util.Counter()
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            oldState = self.setGhostPosition(gameState, oldPos)
            posDist = self.getPositionDistribution(oldState)
            for newPos, prob in posDist.items():
                weighted_prob = self.beliefs[oldPos] * prob
                allPossible[newPos] += weighted_prob
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            oldstate = self.setGhostPosition(gameState, oldPos)
            newPosDist = self.getPositionDistribution(oldstate)
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                posDist = getPositionDistributionForGhost(state, i, self.ghostAgents[i])
