...
public class HashMap<K, V> implements HashMapInterface<K, V> {
    ...
    public V add(K key, V value) {
        if (key == null) {
            throw new IllegalArgumentException("Cannot add entry with null "
                    + "key to the hash map.");
        }
    ...
    public V get(K key) {
        if (key == null) {
            throw new IllegalArgumentException("Cannot get null key from "
                    + "hash map.");
        }
        ...
        do {
            if (table[hash] == null) {
                throw new NoSuchElementException("Could not find inputted key"
                        + " within the hash map.");
            }
            if (table[hash].getKey().equals(key)) {
                return table[hash].getValue();
