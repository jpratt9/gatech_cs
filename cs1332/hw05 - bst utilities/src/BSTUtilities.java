...
public class BSTUtilities {
    ...
    public BSTUtilities(int[] nums, int start, int end) {
        if (start < 0) {
            throw new ArrayIndexOutOfBoundsException("Start index for new BST "
                    + "must be non-negative.");
        }

        if (end > nums.length) {
            throw new ArrayIndexOutOfBoundsException("End index for new BST "
                    + "cannot be greater than length of input array.");
        }

        if (start > end) {
            throw new IllegalArgumentException("Start index for new BST "
    ...
    private int thresholdCount(BSTNode current, int data) {
        if (current == null
                || (current.getData() <= data && current.getRight() == null)) {
            return 0;
        }

        if (current.getData() <= data && current.getRight() != null) {
            return thresholdCount(current.getRight(), data)
                    + thresholdCount(current.getLeft(), data);
        }

        return 1 + thresholdCount(current.getRight(), data)
                + thresholdCount(current.getLeft(), data);
    }
    ...
    private int sumEvenLevels(BSTNode current) {
        ...
        if (current.getLeft() != null) {
            sum = sum + sumEvenLevels(current.getLeft().getLeft())
                    + sumEvenLevels(current.getLeft().getRight());
        }
        if (current.getRight() != null) {
            sum = sum + sumEvenLevels(current.getRight().getLeft())
                    + sumEvenLevels(current.getRight().getRight());
        }
        return sum + current.getData();
    ...
    private Integer getMin(BSTNode current) {
        ...
        if (current.getLeft() == null) {
            return new Integer(current.getData());
        }
    ...
    private int height(BSTNode current) {
        ...
        return Math.max(height(current.getLeft()), height(current.getRight()))
                + 1;
    }
